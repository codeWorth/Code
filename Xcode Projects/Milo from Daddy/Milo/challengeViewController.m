...
-(void)updateTime{
    if (self.timeLeft <= 0) {
        if (self.timeLeft == 0) {
            self.alertBox = [[UIAlertView alloc] initWithTitle:@"Score" message:[NSString stringWithFormat:@"You got %li points!", self.score - self.scoreBeforeStart] delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [self.alertBox show];
        }
...
-(void)dismissAlertBox{
    [self.alertBox dismissWithClickedButtonIndex:0 animated:YES];
    self.alertBox = nil;
}
