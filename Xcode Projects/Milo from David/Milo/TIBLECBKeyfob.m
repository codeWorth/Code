...
//  Milo
//
//  Created by Programming Account on 2/6/15.
...
- (int) findBLEPeripherals:(int) timeout {
    ...
    [self.delegate.TIBLEConnectBtn setTitle:@"Scanning.." forState:UIControlStateNormal];
...
- (void) connectPeripheral:(CBPeripheral *)peripheral {
    ...
    [self.delegate.TIBLEConnectBtn setTitle:@"Connecting.." forState:UIControlStateNormal];
}
...
- (void) printKnownPeripherals {
    ...
    for (i=0; i < self.peripherals.count; i++)
    {
        ...
        CFStringRef s = (__bridge CFStringRef)p.identifier.UUIDString;
...
-(void) getAllServicesFromKeyfob:(CBPeripheral *)p{
    [self.delegate.TIBLEConnectBtn setTitle:@"Discovering services.." forState:UIControlStateNormal];
...
-(void) getAllCharacteristicsFromKeyfob:(CBPeripheral *)p{
    [self.delegate.TIBLEConnectBtn setTitle:@"Discovering characteristics.." forState:UIControlStateNormal];
...
-(const char *) UUIDToString:(NSUUID *)UUID {
    if (!UUID) return "NULL";
    CFStringRef s = (__bridge CFStringRef)UUID.UUIDString;
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
     ...
     */
    if ([peripheral.name rangeOfString:@"MILO"].location != NSNotFound) {
