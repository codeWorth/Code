...
{
    if ([gameMode isEqualToString:@"Zen"] || [gameMode isEqualToString:@"Accuracy"]) {
        _gameMode = gameMode;
    }
    [self startOver];
}
...
- (void)startNewRound
{
    if(round >= 10 && [self.gameMode isEqualToString:@"Accuracy"]){
        NSString *message = [NSString stringWithFormat:@""];
        NSString *title = [NSString stringWithFormat:@""];
        if (score > highscore) {
            highscore = score;
            message = [NSString stringWithFormat:@"Your new highscore is %d!",highscore];
            title = [NSString stringWithFormat:@"Contrats!"];
        } else {
            message = [NSString stringWithFormat:@"You did not beat your highscore of %d.",highscore];
            title = [NSString stringWithFormat:@"Too Bad."];
        }
...
{
    if([[alertView buttonTitleAtIndex:alertView.cancelButtonIndex] isEqualToString:@"OK"]){
        [self startNewRound];
        [self updateLabels];
    } else {
        [self startOver];
